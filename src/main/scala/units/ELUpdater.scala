...
class ELUpdater(
  ...
  private def callContract(fc: FUNCTION_CALL, payload: ExecutionPayload, invoker: KeyPair): Either[String, Unit] = {
    ...
    broadcastTx(tx).resultE match {
      ...
      case Left(TransactionValidationError(InvokeRejectError(message, _), _)) =>
        ...
        s"Failed tx=${tx.id()}: $message".asLeft

      case Left(e) => s"Failed tx=${tx.id()}: ${e.toString}".asLeft
    }
  }
        ...
        getWaitingTime match {
          ...
          case _ =>
            (for {
              ...
              latestValidHash    <- Either.fromOption(latestValidHashOpt, "Latest valid hash not defined")
              _ = logger.info(s"Applied payload $payloadId, block hash is $latestValidHash, timestamp = $timestamp")
              newPm       <- payloadObserver.broadcastSigned(payloadJson, m.keyPair.privateKey)
              payloadInfo <- newPm.payloadInfo
              ...
              err => logger.error(s"Failed to forge block for payloadId $payloadId at epoch ${epochInfo.number}: $err"),
  ...
  private def rollbackTo(
  ...
  ): Either[String, Working[ChainStatus]] = {
    ...
    for {
      ...
      _ <- Either.cond(
        ...
        (),
        s"Rollback to $targetHash error: last block hash ${lastPayload.hash} is not equal to target block hash"
      )
    } yield {
    ...
    wallet.privateKeyAccount(epochInfo.miner) match {
      case Right(keyPair) if config.miningEnable =>
        ...
        (for {
          ...
          err => logger.error(s"Error starting payload build process: $err"),
          _ => ()
        )
      case _ =>
    ...
    state match {
      case w @ Working(epochInfo, _, finalizedBlock, _, _, m: Mining, _, _) if epochInfo.number == epochNumber && blockchain.height == epochNumber =>
        ...
        startBuildingPayload(
          ...
          err => {
            logger.error(s"Error starting payload build process: $err")
  ...
  private def requestBlockPayload(contractBlock: ContractBlock): PayloadRequestResult = {
    ...
    engineApiClient.getBlockByHash(contractBlock.hash) match {
      ...
      case Left(err) =>
        logger.warn(s"Failed to get block ${contractBlock.hash} payload by hash: $err")
        requestAndProcessBlockPayload(contractBlock.hash)
        PayloadRequestResult.Requested(contractBlock)
    }
  }
            ...
            requestBlockPayload(returnToMainChainInfo.missedBlock) match {
              case PayloadRequestResult.Exists(payload) =>
                ...
                validateAppliedBlock(returnToMainChainInfo.missedBlock, payload, w) match {
                  ...
                  case Left(err) =>
                    logger.debug(s"Missed block ${payload.hash} of main chain ${returnToMainChainInfo.chainId} validation error: $err")
                }
  ...
  private def updateToFollowChain(
    ...
    def findLastPayload(curBlock: ContractBlock): ExecutionPayload = {
      engineApiClient.getBlockByHash(curBlock.hash) match {
        ...
        case Left(err) =>
          logger.warn(s"Failed to get block ${curBlock.hash} payload by hash: $err")
          finalizedBlockPayload
      }
    }
    ...
    def rollbackAndFollowChain(
      ...
      rollbackTo(prevState, target, finalizedContractBlock) match {
        ...
        case Left(err) =>
          logger.error(s"Failed to rollback to ${target.hash}: $err")
          None
      }
    }
  ...
  private def waitForSyncCompletion(target: ContractBlock): Unit = scheduler.scheduleOnce(5.seconds)(state match {
    case SyncingToFinalizedBlock(finalizedBlockHash) if finalizedBlockHash == target.hash =>
      ...
      engineApiClient.getLatestBlock match {
        case Left(error) =>
          logger.error(s"Sync to ${target.hash} was not completed, error=$error")
  ...
  private def validateRandao(payload: ExecutionPayload, epochNumber: Int): Either[String, Unit] =
    blockchain.vrf(epochNumber) match {
      ...
      case Some(vrf) =>
        ...
        Either.cond(
          ...
          (),
          s"expected prevRandao $expectedPrevRandao, got ${payload.prevRandao}, VRF=$vrf of $epochNumber"
        )
    }

  private def validateMiner(epi: ExecutionPayloadInfo, epochInfo: Option[EpochInfo]): Either[String, Unit] = {
    val payload = epi.payload
    epochInfo match {
      case Some(epochMeta) =>
        Either.cond(
          ...
          (),
          s"block miner ${payload.feeRecipient} doesn't equal to ${epochMeta.rewardAddress}"
        )
      case _ => Either.unit
    }
  }

  private def validateTimestamp(payload: ExecutionPayload, parentPayload: ExecutionPayload): Either[String, Unit] = {
  ...
  private def getAltChainReferenceBlock(nodeChainInfo: ChainInfo, lastContractBlock: ContractBlock): Either[String, ContractBlock] = {
    if (nodeChainInfo.isMain) {
      for {
        ...
        prevEpoch <- chainContractClient
          ...
          .toRight(s"Can't find a previous epoch #${lastEpoch.prevEpoch} metadata on contract")
        referenceBlockHash = prevEpoch.lastBlockHash
        referenceBlock <- chainContractClient
          .getBlock(referenceBlockHash)
          .toRight(s"Can't find a last block $referenceBlockHash of epoch #${lastEpoch.prevEpoch} on contract")
      ...
      chainContractClient
        .getBlock(blockHash)
        .toRight(
          s"Parent block $blockHash for first block ${nodeChainInfo.firstBlock.hash} of chain ${nodeChainInfo.id} not found at contract"
        )
    }
  }
  ...
  private def validateAndApplyMissed(
    ...
    validateBlockFull(epi, contractBlock, parentPayload, prevState) match {
      ...
      case Left(err) =>
        logger.debug(s"Missed block ${payload.hash} of main chain ${nodeChainInfo.id} validation error: $err, ignoring block")
    }
  }
    ...
    chainContractClient.getBlock(payload.hash) match {
      case Some(contractBlock) if prevState.fullValidationStatus.lastValidatedBlock.hash == parentPayload.hash =>
        ...
        validateBlockFull(epi, contractBlock, parentPayload, prevState) match {
          ...
          case Left(err) =>
            logger.debug(s"Block ${payload.hash} validation error: $err")
  ...
  private def confirmBlockAndFollowChain(
    ...
    confirmBlock(payload, finalizedBlock)
      .fold[Unit](
        err => logger.error(s"Can't confirm block ${payload.hash} of chain ${nodeChainInfo.id}: $err"),
        _ => {
            ...
            case PayloadRequestResult.Exists(payload) =>
              ...
              confirmBlock(payload, finalizedBlock) match {
                case Right(_) =>
                  ...
                  maybeRequestNextBlockPayload(newState, finalizedBlock)
                case Left(err) =>
                  logger.error(s"Failed to confirm next block ${payload.hash}: $err")
                  prevState
              }
  ...
  private def mkRollbackBlock(rollbackTargetBlockHash: BlockHash): Either[String, RollbackBlock] = for {
    ...
    targetBlockData <- Either.fromOption(
      targetBlockDataOpt,
      s"Can't find block $rollbackTargetBlockHash neither on a contract, nor in EC"
    )
    parentPayloadOpt <- engineApiClient.getBlockByHash(targetBlockData.parentHash)
    parentPayload    <- Either.fromOption(parentPayloadOpt, s"Can't find block $rollbackTargetBlockHash parent payload in execution client")
    rollbackBlockOpt <- engineApiClient.applyNewPayload(EmptyPayload.mkExecutionPayloadJson(parentPayload))
    rollbackBlock    <- Either.fromOption(rollbackBlockOpt, "Rollback block hash is not defined as latest valid hash")
  ...
  private def getLastWithdrawalIndex(hash: BlockHash): Either[String, WithdrawalIndex] =
    engineApiClient.getBlockByHash(hash).flatMap {
      case None => Left(s"Can't find block $hash payload on EC during withdrawal search")
      case Some(payload) =>
  ...
  private def getE2CTransfersRootHash(hash: BlockHash, elBridgeAddress: EthAddress): Either[String, Digest] =
    for {
    ...
    state match {
      case w: Working[ChainStatus] =>
        ...
        getContractBlocksForValidation(startState).fold[Unit](
          ...
          blocksToValidate =>
            blocksToValidate.foldLeft[Either[String, Working[ChainStatus]]](Right(startState)) {
              case (Right(curState), block) =>
                ...
                validateAppliedBlock(block.contractBlock, block.payload, curState) match {
                  case Right(updatedState) =>
                    ...
                    Right(updatedState)
                  case Left(err) =>
                    logger.debug(s"Validation of applied block ${block.hash} failed: $err")
  ...
  private def validateE2CTransfers(contractBlock: ContractBlock, elBridgeAddress: EthAddress): Either[String, Unit] =
  ...
  private def validateWithdrawals(
      ...
      fullValidationStatus: FullValidationStatus,
      chainContractOptions: ChainContractOptions
  ): Either[String, Option[WithdrawalIndex]] = {
      ...
      case Some(chainInfo) if canSupportAnotherAltChain(chainInfo) =>
        (for {
          ...
          lastValidBlock <- chainContractClient
            ...
            .toRight(s"Block ${updatedState.lastPayload.hash} not found at contract")
        } yield {
  ...
  private def getContractBlocksForValidation(curState: Working[ChainStatus]): Either[String, List[BlockForValidation]] = {
    @tailrec
    def loop(curBlock: ContractBlock, acc: List[BlockForValidation]): Either[String, List[BlockForValidation]] = {
          ...
          case Some(parentBlock) =>
            if (curBlock.height > curState.lastPayload.height) {
              ...
              engineApiClient.getBlockByHash(curBlock.hash) match {
                ...
                case Right(None) =>
                  Left(s"Block ${curBlock.hash} payload not found on EC client for full validation")
                case Left(err) =>
                  Left(s"Can't get block ${curBlock.hash} payload for full validation: $err")
              }
            }
          case _ =>
            Left(s"Block ${curBlock.parentHash} not found at contract during full validation")
        }
      }
    }
  ...
  private def confirmBlock(blockData: CommonBlockData, finalizedBlockData: CommonBlockData): Either[String, PayloadStatus] = {
  ...
  private def confirmBlock(hash: BlockHash, finalizedBlockHash: BlockHash): Either[String, PayloadStatus] =
